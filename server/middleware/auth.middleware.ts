...
export const authenticateTokenAdmin = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    // Xác định role từ token type (ưu tiên content nếu có token/cookie content)
    if (req.cookies.access_token_content || req.cookies.refresh_token_content) {
      tokenRole = 'content'
    }
    ...
    if (!accessToken) {
      ...
      CookieUtil.setAccessTokenCookie(
        res,
        newAccessToken,
        tokenRole as 'admin' | 'user' | 'content',
      )
...
export const requireContentPermission = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    if (!req.user) return next(new ErrorHandler('Vui lòng đăng nhập', 401))
...
async function updateLastActiveDateForUser(userId: string): Promise<void> {
  await User.updateOne({ _id: userId }, { lastActiveDate: new Date() })
}
...
export const updateLastActiveDate = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
