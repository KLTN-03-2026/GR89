...
export class DailySuggestionService {
  static async getDailySuggestion(userId: string) {
    ...
    if (!suggestion) {
      ...
      const userPrompt =
        'Hãy gợi ý cho tôi 5-8 bài học tiếp theo dựa trên tiến trình và lịch sử học tập của tôi.'

      let validSuggestions = []
      try {
        const aiResponse = await AIProvider.evaluateEssay<IDailySuggestionItem[]>(
          systemPrompt,
          userPrompt,
        )
    ...
    const enrichedSuggestions = await Promise.all(
      suggestion.suggestions.map(async (item) => {
