...
export class AuthController {
  ...
  // QUÊN MẬT KHẨU
  static requestPasswordReset = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
  ...
  static resetPassword = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!token || !password || !confirmPassword) return next(new ErrorHandler('Thiếu dữ liệu', 400));
    if (password !== confirmPassword) return next(new ErrorHandler('Mật khẩu không khớp', 400));
  ...
  static login = CatchAsyncError(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      if (role != 'user' && role != 'admin' && role != 'content') return next(new ErrorHandler('Vui lòng chọn vai trò', 400))
  ...
  static loginAdmin = CatchAsyncError(
    async (req: Request, res: Response, next: NextFunction) => {
