...
export default function CardIpa({ sound, soundType, examples, progress, _id, isVipRequired }: props) {
  ...
  return (
    <Link
      ...
      className={`hover:scale-[1.02] cursor-pointer w-full min-h-[100px] md:min-h-[120px] flex flex-col justify-center items-center p-1 md:p-2 lg:p-3 rounded-xl border border-border/50 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 hover:shadow-md transition-all duration-300 relative overflow-visible ${borderTypeColor[soundType]}`}
    >
      {isVipRequired && (
        <div className="absolute -top-1 -right-1 z-30 bg-yellow-500 rounded-full p-0.5 md:p-1 shadow-lg border-2 border-white">
