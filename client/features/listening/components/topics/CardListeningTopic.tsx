...
import { Star, Headphones, Crown } from "lucide-react";
  ...
  return (
    <Popover>
      <PopoverTrigger className='w-full text-center group'>
        <Card className='relative flex flex-col h-full justify-between transition-all duration-500 ease-out p-6 bg-white border-2 border-emerald-300 shadow-xl hover:shadow-2xl hover:scale-105 overflow-hidden'>
          <CardHeader className="relative z-10 p-0">
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                {level ? (
                  <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-pink-50 text-pink-700 border border-pink-200">
                    {level}
                  </span>
                ) : null}
                {topic.isVipRequired && (
          ...
          <CardContent className="p-0">
            <CardTitle className='font-bold text-lg mb-2 transition-all duration-300 text-gray-800 group-hover:text-pink-600'>{topic.title}</CardTitle>
          ...
          <CardFooter className="p-0 space-y-3 flex flex-col">
            <Progress value={topic.progress} className="bg-emerald-100! [&>div]:bg-emerald-500!" />

            <div className="flex justify-between text-xs w-full">
              <span className="text-gray-600">Tiến độ nghe</span>
