...
export function FlowHeader({ title, stage }: FlowHeaderProps) {
  const getStageConfig = () => {
    switch (stage) {
      ...
      case 'quiz':
        return { label: 'KIỂM TRA', icon: GraduationCap, color: 'bg-amber-50 text-amber-600 border-amber-100' }
      default:
        return { label: 'HOÀN THÀNH', icon: CheckCircle2, color: 'bg-blue-50 text-blue-600 border-blue-100' }
    }
  }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">
            Lộ trình học tối ưu: <span className="text-emerald-500">Lý thuyết</span> → <span className="text-purple-500">Luyện tập</span> → <span className="text-amber-500">Kiểm tra</span>
          </p>
        </div>
