  ...
  return (
    <>
      <section className="mb-5">
        <div className={`flex items-center gap-4 justify-between w-full rounded-2xl px-8 py-12 bg-gradient-to-br from-teal-500 via-cyan-500 to-blue-200`}>
          ...
          <div className="hidden xl:block w-[280px] h-[180px] bg-teal-600/30 rounded-xl" />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 mt-4">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
              <Skeleton className="h-8 w-8 bg-teal-100 dark:bg-teal-900/30 rounded-lg" />
      ...
      <section className="space-y-6">
        <div className="flex items-center space-x-3">
          <div className="w-8 h-8 bg-teal-200 rounded-lg animate-pulse"></div>
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(250px,1fr))] gap-4">
          {Array.from({ length: 6 }).map((_, index) => (
            ...
            >
              <div className="w-full h-40 bg-teal-100 dark:bg-teal-900/30 rounded-lg"></div>
              <div className="w-3/4 h-5 bg-gray-200 dark:bg-gray-700 rounded"></div>
              <div className="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full">
                <div className="w-1/3 h-2 bg-teal-200 dark:bg-teal-800 rounded-full"></div>
              </div>
            </div>
          ))}
        </div>
