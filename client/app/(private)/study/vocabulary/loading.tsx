  ...
  return (
    <>
      <section className="mb-5">
        <div className={`flex items-center gap-4 justify-between w-full rounded-2xl px-8 py-12 bg-gradient-to-br from-emerald-500 via-green-400 to-teal-200`}>
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(240px,1fr))] gap-4 items-stretch">
          {Array.from({ length: 6 }).map((_, index) => (
            <div
              key={index}
              className="bg-white dark:bg-gray-800 rounded-xl p-4 space-y-4 animate-pulse h-full flex flex-col"
            >
              <div className="w-full h-40 bg-emerald-100 dark:bg-emerald-900/30 rounded-lg"></div>
              <div className="w-3/4 h-5 bg-gray-200 dark:bg-gray-700 rounded"></div>
              <div className="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full">
                <div className="w-1/3 h-2 bg-emerald-200 dark:bg-emerald-800 rounded-full"></div>
              </div>
            </div>
          ))}
        </div>
