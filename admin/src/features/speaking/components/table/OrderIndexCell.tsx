...
import { swapSpeakingOrder } from "@/features/speaking/services/api"
...
export default function OrderIndexCell({ speaking, allSpeakings, callback, onSwap }: OrderIndexCellProps) {
  const [isLoading, setIsLoading] = useState(false)
  const [isAnimating, setIsAnimating] = useState(false)

  const orderIndexes = allSpeakings.map(s => s.orderIndex).sort((a, b) => a - b)
  const minOrderIndex = orderIndexes[0]
  const maxOrderIndex = orderIndexes[orderIndexes.length - 1]

  const canMoveUp = speaking.orderIndex > minOrderIndex
  const canMoveDown = speaking.orderIndex < maxOrderIndex
  ...
  const handleSwap = async (direction: 'up' | 'down') => {
    ...
    setIsLoading(true)
    try {
      await swapSpeakingOrder(speaking._id, direction)
      toast.success(`Đã di chuyển speaking ${direction === 'up' ? 'lên' : 'xuống'} thành công`)
      if (!onSwap) {
        callback()
      }
    } catch (error: unknown) {
