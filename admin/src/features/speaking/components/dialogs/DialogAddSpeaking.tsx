...
export function DialogAddSpeaking({ callback }: { callback: () => void }) {
      ...
      <SheetContent className="h-full sm:max-w-3xl flex flex-col p-0 border-l shadow-2xl overflow-hidden">
        <SheetHeader className="p-8 pb-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <SheetTitle className="text-2xl font-black text-gray-900 tracking-tight">Thiết Kế Bài Nói</SheetTitle>
              <SheetDescription className="text-gray-500 font-medium mt-1">
                Tạo bài tập luyện nói với video hướng dẫn chuyên sâu.
              </SheetDescription>
            </div>
          </div>
        </SheetHeader>
        ...
        <SheetFooter className="p-8 bg-gray-50/80 backdrop-blur-sm border-t border-gray-100">
          <div className="flex items-center justify-end gap-4 w-full">
            ...
            <Button className="h-12 px-10 rounded-2xl bg-primary hover:bg-primary/90 shadow-xl shadow-primary/20 font-black transition-all active:scale-95 text-white" onClick={handleCreateSpeaking} disabled={isLoading}>
              {isLoading ? (
                <>
