...
interface Props {
  ...
  urlSortBy: 'orderIndex' | 'title' | 'createdAt' | 'updatedAt'
  ...
  speakingsLength: number
  total: number
}

export default function FiltersPanel({ urlIsActive, urlSortBy, urlSortOrder, urlLimit, updateUrl, search, setSearch, handleSearch, activeFiltersCount, setShowFilters, speakingsLength, total }: Props) {
    ...
    <div className="mb-6 p-4 bg-gray-50 rounded-lg border">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Select value={urlSortBy} onValueChange={(value: 'orderIndex' | 'title' | 'createdAt' | 'updatedAt') => handleSort(value)}>
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
      ...
      <div className="flex items-center justify-between mt-4 pt-4 border-t">
        ...
        <div className="text-sm text-gray-500">
          Hiển thị {speakingsLength} trong {total} kết quả
        </div>
      </div>
    </div>
  )
