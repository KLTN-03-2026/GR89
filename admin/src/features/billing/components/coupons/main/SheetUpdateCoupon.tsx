...
import { updateCoupon, Plan, Coupon } from "@/lib/apis/api"
import { toast } from "react-toastify"

interface SheetUpdateCouponProps {
  open: boolean
...
export function SheetUpdateCoupon({ open, setOpen, coupon, plans, callback }: SheetUpdateCouponProps) {
  ...
  useEffect(() => {
    if (coupon) {
      ...
      setSelectedPlanIds(Array.isArray(coupon.applicablePlans) ? coupon.applicablePlans : [])
    }
  }, [coupon])
              ...
              <div className="grid grid-cols-2 gap-6 bg-gray-50/50 p-6 rounded-[2rem] border border-gray-100 shadow-sm">
                <div className="space-y-2.5">
                  ...
                  <Input
                    ...
                    disabled
                    className="h-12 bg-white border-gray-200 rounded-2xl focus:ring-rose-500 font-black px-4 shadow-sm uppercase opacity-70"
                  />
                </div>
        ...
        <SheetFooter className="p-8 bg-gray-50/80 backdrop-blur-sm border-t border-gray-100">
          <div className="flex items-center justify-end gap-4 w-full">
            ...
            >
              ...
              {loading ? 'Đang Xử Lý...' : 'Cập Nhật Mã'}
            </Button>
          </div>
        </SheetFooter>
      </SheetContent>
