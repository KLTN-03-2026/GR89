...
import { createPlan, Plan } from '@/lib/apis/api'
...
interface SheetAddPlanProps {
  callback?: () => void
}

export function SheetAddPlan({ callback }: SheetAddPlanProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success('Tạo gói nâng cấp thành công')
      callback?.()
      setOpen(false)
      setDraft({
        name: "",
  ...
  return (
    <Sheet open={open} onOpenChange={(isOpen) => {
      ...
      <SheetTrigger asChild>
        <Button className="h-12 px-6 rounded-xl bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-200 transition-all font-bold">
          <Plus className="w-5 h-5 mr-2" /> Thêm Gói Mới
        </Button>
      </SheetTrigger>
      <SheetContent className="sm:max-w-2xl h-full flex flex-col p-0 border-l shadow-2xl overflow-hidden">
        <SheetHeader className="p-8 pb-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <SheetTitle className="text-2xl font-black text-gray-900 tracking-tight">Tạo Gói Nâng Cấp</SheetTitle>
              <SheetDescription className="text-gray-500 font-medium mt-1">
                Thiết kế gói dịch vụ mới cho người dùng của bạn.
              </SheetDescription>
            </div>
          </div>
        </SheetHeader>
              ...
              <div className="grid grid-cols-2 gap-6 bg-gray-50/50 p-6 rounded-[2rem] border border-gray-100 shadow-sm">
                ...
                <div className="space-y-2.5 col-span-2 md:col-span-1">
                  ...
                  <Select
                    ...
                    onValueChange={(v: "default" | "vip" | "premium") => setDraft({ ...draft, displayType: v })}
                  >
              ...
              <div className="grid grid-cols-2 gap-6 bg-blue-50/30 p-6 rounded-[2rem] border border-blue-100/50">
                ...
                <div className="space-y-2.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v: "monthly" | "yearly" | "lifetime") => setDraft({ ...draft, billingCycle: v })}
                  >
                ...
                {draft.discountPercent && draft.discountPercent > 0 && draft.originalPrice && (
                  <div className="col-span-2 mt-2 p-5 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-3xl shadow-lg shadow-blue-100 text-white animate-in fade-in slide-in-from-top-4 duration-500">
                    <div className="flex items-center justify-between mb-2 opacity-90 text-xs font-bold uppercase tracking-widest">
                      <span>Giá Niêm Yết Cuối Cùng</span>
                    ...
                    <div className="mt-3 flex items-center gap-2 text-[11px] font-bold bg-black/10 p-2 rounded-xl">
                      ...
                      Giảm trực tiếp {Math.round(Number(draft.originalPrice) * (draft.discountPercent / 100)).toLocaleString('vi-VN')}₫ so với giá gốc
                    </div>
                  </div>
                )}
              </div>
            ...
            <section className="space-y-6 pb-10">
              ...
              <div className="space-y-4 bg-amber-50/30 p-6 rounded-[2rem] border border-amber-100/50">
                ...
                <div className="flex items-start gap-2 text-[11px] text-amber-700/70 font-bold bg-amber-100/40 p-3 rounded-xl">
                  ...
                  Mẹo: Mỗi dòng sẽ hiển thị dưới dạng một dấu tích (v) trong bảng giá của người dùng. Hãy viết ngắn gọn và hấp dẫn!
                </div>
              </div>
            </section>
          </div>
        ...
        <SheetFooter className="p-8 bg-gray-50/80 backdrop-blur-sm border-t border-gray-100">
          <div className="flex items-center justify-end gap-4 w-full">
            ...
            >
              {loading ? 'Đang Khởi Tạo...' : 'Phát Hành Gói'}
            </Button>
          </div>
        </SheetFooter>
      </SheetContent>
