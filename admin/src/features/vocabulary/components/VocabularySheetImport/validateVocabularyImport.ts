...
interface DataImportTopic {
  name: string
  image: string
  level?: string
  words?: DataImportWord[]
  quizzes?: DataImportQuiz[]
  isRequired?: boolean
}

interface DataImportWord {
  word: string
...
export function validateVocabularyImportJson(input: unknown): ParseResult {
  ...
  input.forEach((topic, i) => {
    ...
    const t = topic as DataImportTopic
    ...
    t.words?.forEach((w: DataImportWord, j: number) => {
    ...
    t.quizzes?.forEach((q: DataImportQuiz, j: number) => {
  ...
  return { ok: true, data: input as DataImportTopic[] }
}
...
export function parseExcelToVocabularyJson(sheets: Sheet[]): ParseResult {
  ...
  const topicMap = new Map<string, DataImportTopic>()
