...
import { swapListeningOrder } from "@/features/listening/services/api"
...
export default function OrderIndexCell({ listening, allListenings, callback, onSwap }: OrderIndexCellProps) {
  const [isLoading, setIsLoading] = useState(false)
  const [isAnimating, setIsAnimating] = useState(false)

  const orderIndexes = allListenings.map(l => l.orderIndex).sort((a, b) => a - b)
  const minOrderIndex = orderIndexes[0]
  const maxOrderIndex = orderIndexes[orderIndexes.length - 1]

  const canMoveUp = listening.orderIndex > minOrderIndex
  const canMoveDown = listening.orderIndex < maxOrderIndex
  ...
  const handleSwap = async (direction: 'up' | 'down') => {
    ...
    if (onSwap) {
      setIsAnimating(true)
      onSwap(listening._id, direction)
      setTimeout(() => {
    ...
    setIsLoading(true)
    try {
      await swapListeningOrder(listening._id, direction)
      toast.success(`Đã di chuyển listening ${direction === 'up' ? 'lên' : 'xuống'} thành công`)
      if (!onSwap) {
        callback()
      }
    } catch (error: unknown) {
